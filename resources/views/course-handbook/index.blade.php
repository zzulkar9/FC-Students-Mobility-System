                            ...
                            <div x-show="activeIntake === '{{ $intake }}'">
                                <h3
                                    ...
                                    <form method="POST" action="{{ route('notes.store') }}" class="inline">
                                        ...
                                        <textarea name="note" class="w-full rounded-md border-gray-300 shadow-sm"
                                            placeholder="Enter note for {{ $intake }} here...">{{ isset($notes["$year-$intake"]) && $notes["$year-$intake"]->firstWhere('year_semester', null) ? $notes["$year-$intake"]->firstWhere('year_semester', null)->note : '' }}</textarea>
                                        <button type="submit"
                                            class="mt-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-1 px-4 rounded">Save
                                            Note</button>
                                    </form>
                                </h3>
                                            ...
                                            <td colspan="4" class="px-6 py-4">
                                                <form method="POST" action="{{ route('notes.store') }}">
                                                    ...
                                                    <textarea name="note"
                                                        ...
                                                        placeholder="Enter note here...">{{ isset($notes["$year-$intake-$semester"]) ? $notes["$year-$intake-$semester"]->first()->note : '' }}</textarea>
                                                    <button type="submit"
                                                        class="mt-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-1 px-4 rounded">Save
                                                        Note</button>
                                                </form>
    ...
    <div class="py-12">
        <div class="flex">
            ...
            <div class="w-1/6 bg-gray-200 p-4 border-r border-gray-300">
                <button class="tab-button active-tab w-full px-4 py-2 mb-2 rounded-lg hover:bg-blue-500"
                    data-tab="view-tab-content">View</button>
                <button class="tab-button inactive-tab w-full px-4 py-2 mb-2 rounded-lg hover:bg-blue-200"
                    data-tab="edit-tab-content">Edit</button>
                <a href="{{ route('courses.create') }}"
                    class="inactive-tab w-full px-4 py-2 rounded-lg block text-center mt-2 bg-blue-500 hover:bg-blue-700 text-white">Add</a>
            </div>
            ...
            <div class="w-5/6 bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-b border-gray-200">
                <div id="view-tab-content" class="tab-content active">
                    @include('course-handbook.course-handbook-partials.view', ['years' => $years,
                    'coursesByYearAndIntake' => $coursesByYearAndIntake, 'totalCreditsBySemester' =>
                    $totalCreditsBySemester, 'notes' => $notes])
                </div>
                <div id="edit-tab-content" class="tab-content">
                    @include('course-handbook.course-handbook-partials.edit', ['years' => $years,
                    'coursesByYearAndIntake' => $coursesByYearAndIntake, 'totalCreditsBySemester' =>
                    $totalCreditsBySemester, 'notes' => $notes])
                </div>
            </div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="tabs mb-8 flex justify-center border-b-2 border-gray-200">
                <button class="tab-button active" data-tab="view-tab-content">View Course Menu</button>
                <button class="tab-button" data-tab="edit-tab-content">Edit Course Menu</button>
                <button class="tab-button" data-tab="year-tab-content">Duplicate Course Menu (Yearly)</button>
                <button class="tab-button" data-tab="add-tab-content">Add Course Menu</button>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-b border-gray-200">
                ...
                <div id="add-tab-content" class="tab-content">
                    @include('course-handbook.course-handbook-partials.create')
                </div>
            </div>
        </div>
    </div>
