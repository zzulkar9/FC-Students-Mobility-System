                        ...
                        @foreach ($intakes as $intake => $semesters)
                            <div x-show="activeIntake === '{{ $intake }}'">
                                ...
                                <div class="bg-gray-100 p-4 rounded-lg mb-6">
                                    <strong>Note for {{ $intake }}:</strong>
                                    <p>{{ isset($notes["$year-$intake"]) && $notes["$year-$intake"]->firstWhere('year_semester', null) ? $notes["$year-$intake"]->firstWhere('year_semester', null)->note : 'No notes available.' }}
                                    </p>
                                </div>
                                @foreach ($semesters as $semester => $courses)
                                    <h4 class="text-md font-semibold text-gray-700 mt-4 mb-2">
                                        {{ $semester }}
                                    </h4>
                                    <table class="min-w-full mb-6 border-collapse border border-gray-300">
                                        <thead>
                                            <tr>
                                                <th
                                                    class="px-6 py-3 bg-gray-200 text-left text-xs leading-4 font-medium text-gray-900 uppercase tracking-wider border border-gray-300">
                                                    Course Code
                                                </th>
                                                <th
                                                    class="px-6 py-3 bg-gray-200 text-left text-xs leading-4 font-medium text-gray-900 uppercase tracking-wider border border-gray-300">
                                        ...
                                        <tbody class="bg-white">
                                            @foreach ($courses as $course)
                                                <tr class="hover:bg-gray-100">
                                                    <td
                                                        class="px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium text-gray-600 border border-gray-300">
                                                        {{ $course->course_code }}
                                                    </td>
                                                    <td
                                                        class="px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium text-gray-600 border border-gray-300">
                                            ...
                                            <tr class="bg-gray-100">
                                                <td colspan="2"
                                                    class="text-right px-6 py-4 font-medium border border-gray-300">
                                                    Total Credits:
                                                </td>
                                                <td class="px-6 py-4 font-medium border border-gray-300">
                                                    {{ $totalCreditsBySemester[$year][$intake][$semester] }}
                                                </td>
                                                <td></td> <!-- Empty cell for alignment -->
                                            </tr>
                                            ...
                                            <tr class="bg-gray-100">
                                                <td colspan="4" class="px-6 py-4 border border-gray-300">
                                                    <div class="p-4 bg-gray-100 rounded-lg">
                                                        <strong>Note for {{ $semester }}:</strong>
                                                        <p>{{ isset($notes["$year-$intake-$semester"]) ? $notes["$year-$intake-$semester"]->first()->note : 'No notes available.' }}
                                                        </p>
                                                    </div>
                                                </td>
                                            </tr>
