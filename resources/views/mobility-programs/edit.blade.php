                    ...
                    <form method="POST" action="{{ route('mobility-programs.update', $program->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <div id="file-info" class="mt-2 text-sm text-gray-600">
                                @if ($program->image)
                                    Current file: {{ $program->image }}
                                @endif
                            </div>
                            @if ($program->image)
                                <img src="{{ asset('storage/' . $program->image) }}" alt="{{ $program->title }}" class="mt-2 w-32 h-32 object-cover rounded-md">
                                <button id="remove-file" type="button" class="mt-2 bg-red-500 text-white py-1 px-2 rounded">Remove File</button>
                            @endif
                            @error('image')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" id="due_date" name="due_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('due_date', $program->due_date) }}">
                            @error('due_date')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
